...
import { X, Minus, Plus, ShoppingBag, CreditCard, Mail } from "lucide-react";
...
export default function CartDrawer({ cart, tableToken, onClose, onUpdateQty, onOrderCreated }: Props) {
  ...
  // Revisar si hay sesión de Google activa al abrir el drawer
  useEffect(() => {
  ...
  function handleGoogleLogin() {
    ...
    supabase.auth.signInWithOAuth({
      provider: "google",
      options: {
        redirectTo: `${window.location.origin}/auth/mesa-callback?next=${encodeURIComponent(currentUrl)}`,
      },
    });
    // La página va a redirigir — el carrito se restaura desde localStorage al volver
  }
  ...
  async function handleConfirm() {
    ...
    if (paymentMode === "CASHIER" && !googleUser) {
      setError("Necesitás iniciar sesión con Google para pagar en caja");
      return;
    }
    ...
    try {
      const order = await createOrder({
        ...
        customerEmail: googleUser?.email ?? (receiptEmail.trim() || undefined),
      });
      ...
      <div className="fixed inset-0 z-50 flex flex-col justify-end" style={{ height: "100dvh" }}>
        ...
        >
          ...
          <div className="overflow-y-auto flex-1 overscroll-contain px-5">
            ...
            {/* Sección dinámica según modo de pago */}
            {paymentMode === "CASHIER" ? (
              <div className="mb-5">
                {googleUser ? (
                  // Ya logueado con Google
                  ...
                  // No logueado — pedir Google login
                  <div className="bg-zinc-50 border border-zinc-100 rounded-2xl p-4">
                    <p className="text-zinc-700 text-sm font-semibold mb-1">Identificate para pagar en caja</p>
                    <p className="text-zinc-400 text-xs mb-4 leading-snug">
                      Para evitar pedidos falsos, necesitamos confirmar que sos una persona real.
                      Solo usamos tu nombre y email para identificar el pedido.{" "}
                    ...
                    <button
                      ...
                      className="w-full flex items-center justify-center gap-3 bg-white hover:bg-zinc-50 active:bg-zinc-100 disabled:opacity-50 border border-zinc-200 text-zinc-800 font-semibold py-3 rounded-xl transition-all text-sm min-h-[48px] shadow-sm"
                    >
                      <GoogleIcon />
              ...
              // ONLINE — email opcional para comprobante
              <div className="mb-5">
                <label className="text-[11px] font-semibold text-zinc-400 uppercase tracking-widest block mb-2">
                  Comprobante (opcional)
                </label>
                <div className="relative">
                  <Mail size={14} className="absolute left-3 top-1/2 -translate-y-1/2 text-zinc-400 pointer-events-none" strokeWidth={2} />
                  <input
                    type="email"
                    value={receiptEmail}
                ...
                <p className="text-[10px] text-zinc-400 mt-1.5 px-1 leading-snug">
                  Si querés recibir el comprobante de pago, ingresá tu email. Es opcional.
                </p>
              </div>
            )}
          </div>
