  ...
  return (
    <AnimatePresence>
      ...
      <div className="fixed inset-0 z-50 flex flex-col justify-end" style={{ height: "100dvh" }}>
        ...
        {/* Drawer — máx 92dvh para no tapar la pantalla en celulares chicos */}
        <motion.div
          initial={{ y: "100%" }}
          animate={{ y: 0 }}
          exit={{ y: "100%" }}
        ...
        >
                ...
                <div key={item.menuItemId} className="flex items-center gap-3 min-h-[48px]">
                  ...
                  <div className="flex items-center gap-2 flex-shrink-0">
                    <button
                      ...
                      className="w-9 h-9 rounded-full border border-zinc-200 flex items-center justify-center active:bg-zinc-50 transition-colors"
                      aria-label="Quitar"
                    >
                      <Minus size={13} strokeWidth={2.5} />
                    </button>
                    <span className="w-5 text-center font-bold text-sm tabular-nums text-zinc-900">
                      {item.quantity}
                    </span>
                    <button
                      onClick={() => onUpdateQty(item.menuItemId, 1)}
                      className="w-9 h-9 rounded-full bg-zinc-900 flex items-center justify-center text-white active:bg-zinc-700 transition-colors"
                      aria-label="Agregar"
                    >
          ...
          {/* Footer — siempre visible, respeta home indicator */}
          <div
            className="flex-shrink-0 px-5 pt-4 border-t border-zinc-100 bg-white"
            style={{ paddingBottom: "max(1.25rem, env(safe-area-inset-bottom))" }}
          >
            {error && (
              <p className="text-red-500 text-xs text-center mb-3 bg-red-50 rounded-lg py-2">
                {error}
              </p>
            )}
    ...
    <button
      ...
      className={`relative p-4 rounded-2xl border-2 text-left transition-all duration-200 min-h-[90px] active:scale-[0.98] ${
        selected
          ...
          : "border-zinc-100 bg-zinc-50 text-zinc-900 active:border-zinc-300"
      }`}
    >
      <div className={`text-xl mb-2 ${selected ? "text-white" : "text-zinc-700"}`}>{icon}</div>
      <p className="font-semibold text-sm leading-tight">{title}</p>
      <p className={`text-xs mt-0.5 leading-tight ${selected ? "text-zinc-300" : "text-zinc-400"}`}>
        {subtitle}
      </p>
    </button>
  );
