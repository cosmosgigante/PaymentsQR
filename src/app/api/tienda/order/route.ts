...
export async function POST(req: NextRequest) {
  ...
  let body: { slug?: string; items?: { menuItemId?: string; quantity?: number; notes?: string }[]; name?: string; notes?: string };
  ...
  const order = await db.order.create({
    data: {
      ...
      tableId: point.id,
      paymentMode: "CASHIER", // pagás al retirar (MVP); pago online = próximo
      total,
      notes,
