...
export async function POST(req: NextRequest) {
  ...
  // Total impago de la sesión.
  const orders = await db.order.findMany({
    where: { tableSessionId: session.id, status: { in: UNPAID } },
    select: { total: true },
  });
  ...
  const pref = await createCheckoutPreference({
    ...
    notificationUrl: `${origin}/api/webhooks/mercadopago?session=${session.id}`,
    backUrl: `${origin}/mesa/${tableToken}?paid=1`,
  });
  if ("error" in pref) return NextResponse.json({ error: pref.error }, { status: 502 });

  await db.tableSession.update({
    where: { id: session.id },
    data: { paymentStatus: "PENDING", paymentPreferenceId: pref.id },
  });
